...
ENTRY_MAX_ATR_DISTANCE=float(os.getenv("ENTRY_MAX_ATR_DISTANCE","1.0"))
...
def qty_ok(s,x):
    ...
    return q
def save():
    with open(STATE,"w") as f: json.dump({"mine":mine,"pause":pause_until,"loss":loss_window,"losing":losing_cycles,"cycle":cycle_realized,"bot_realized":bot_realized,"basket_lock_candle":basket_lock_candle},f)
def load():
    global mine,pause_until,loss_window,losing_cycles,cycle_realized,bot_realized,basket_lock_candle
    try:
        d=json.load(open(STATE)); mine=d.get("mine",{}); pause_until=d.get("pause",0); loss_window=d.get("loss",0); losing_cycles=d.get("losing",0); cycle_realized=d.get("cycle",0); bot_realized=d.get("bot_realized",0); basket_lock_candle=d.get("basket_lock_candle",0)
    except: pass
...
def place_targets(s,direction,entry_px,lev,full_qty):
    ...
    step=meta[s]["step"]
    half=qty_ok(s,full_qty*0.5)
    rest=qty_ok(s,full_qty-half)
    tp1_move=1.00/lev
    tp2_move=1.50/lev
...
def sig(s, entry_filter=False):
    ...
    last=closes[-1]
    m7=sma(closes,7); m25=sma(closes,25); m99=sma(closes,99)

    if last>m99 and last>m25:
        direction="LONG"
    elif last<m99 and last<m25:
        direction="SHORT"
    else:
        return "WAIT"

    # Only new entries use this anti-chase filter.
    # Existing-position exits remain governed by MA25/MA99 exactly as before.
    if entry_filter:
        trs=[]
        ...
        if atr<=0:return "WAIT"
        nearest=min(abs(last-m7),abs(last-m25))
        if nearest > atr*ENTRY_MAX_ATR_DISTANCE:
            return "WAIT"
...
def main():
    ...
    msg(f"MA BTC Sync Bot {BOT_VERSION} STARTED\\n15m SMA 7/25/99 | Allocated: ${ALLOCATED_CAPITAL:.0f} | Notional: $300 | Max: 20 | Basket: NET +$20 AFTER CLOSE FEES | Entries: Liquidity Reversal Staged TRAP/PO3+RSI | BTC MA25/99 direction gate\\nExcluded: BNB, DOGE, BCH | Liquidity floor: ${MIN_VOL:,.0f}/24h")
    last=0
    while True:
        try:
            manage()
